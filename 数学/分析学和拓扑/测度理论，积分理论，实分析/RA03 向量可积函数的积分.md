...
# IV. $\mathcal L_{1}(X,\mu,\mathbb E)$是一个完备向量空间
$\mathcal L_{1}(X,\mu,\mathbb E)$：由如下性质：
## IV.1. $\mathcal L_{1}(X,\mu,\mathbb E)$在函数加法的意义下是$\mathcal L_{0}(X,\mu,\mathbb E)$的子向量空间
>**证明加法封闭**
...
## IV.3. $\mathcal L_{1}$等价 等价于 $\mathcal L_{0}$等价
$|f-g|_{\mathcal L_{1}} = 0$当且仅当$f=g$ a.e
>由非负积分的性质可以简单推出


## IV.4. $\mathcal L_{1}(X,\mu,\mathbb E)$作为半赋范向量空间是完备的
>**找到一个a.e收敛的子列**
...
## IV.5. $\mathcal S(X,\mu,\mathbb E)$是$\mathcal L_{1}(X,\mu,\mathbb E)$的子向量空间

>平凡
## IV.6. $\mathcal S(X,\mu,\mathbb E)$在$\mathcal L_{1}(X,\mu,\mathbb E)$中稠密
...
# VI. $\mathcal L_{1}(X,\mu,\mathbb E)$上的向量值积分
...
## VI.2. 线性空间间的有界线性算子导出$\mathcal L_{1}$空间的有界线性算子
	此处使用了lambda表达式
